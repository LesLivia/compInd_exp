...
from utils.verify import run_exp
...
for n_c, r in itertools.product(N_comp, repl):
    ...
    AUTOMATON = Automaton(NAME.format(n_c, r), AUTOMATON_PATH)
    links_identifier = Identifier(AUTOMATON)
    links = links_identifier.identify_semantic_links(CS + '/' + NAME.format(n_c, r).replace('_source.txt',
                                                                                            ''), '.')
    ...
    sha = parse_sha(AUTOMATON_PATH, NAME.format(n_c, r))
    ...
    for trace in tqdm(validation_traces):
        model_path, query_path = generate_upp_model(sha, NAME.format(n_c, r), trace)
        success = run_exp(NAME.format(n_c, r), model_path, query_path)
        if success:
            compatible_traces += 1
    print('{}/{} compatible traces'.format(compatible_traces, len(validation_traces)))
    with open('{}_res.txt'.format(CS), 'a') as f:
        f.write('{}\t{:.2f}\n'.format(NAME.format(n_c, r), compatible_traces / len(validation_traces)))
